...
// Function Def
void bucketSortParallel(int* arr, int n, int threads_passed, int bucket_size) {
...
#pragma omp parallel for num_threads(threads_passed)
    for (int i = 0; i < bucket_count; i++)
    {
...
int main()
{
    int arr[] = { 8, 15, 10, 13, 0,  110, 11,3, 5, 6, 55, 44, 33, 22, 66, 77, 88, 99, 98, 97, 87, 76, 65, 54, 43 };
    int length_arr = sizeof(arr) / sizeof(arr[0]);

    auto start = chrono::steady_clock::now();
    bucketSortParallel(arr, length_arr, 4, 7);
