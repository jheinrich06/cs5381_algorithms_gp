...
int main()
{
    ...
    */
    ...
    for (int i = 0; n_length - 1; i++) {
        ...
        insertionSortParallel(randomChars_gen_small, randomChars_gen_small_n, 0, randomChars_gen_small_n - 1, 7);
        ...
        start = chrono::steady_clock::now();
        insertionSortParallel(randomChars_gen_large, randomChars_gen_large_n, 0, randomChars_gen_large_n - 1, 7);
        end = chrono::steady_clock::now();
        elapsed = chrono::duration_cast<chrono::nanoseconds>(end - start).count();
        //result_times.push_back(elapsed);
        cout << "LargeChar ";
